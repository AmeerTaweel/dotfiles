{params, pkgs, ...}: let
  mkBorgmaticNotification = {title, urgency ? "NORMAL"}: ''
  ...
  mkBorgmaticHooks = repo: {
    before_backup  = [(mkBorgmaticNotification {
      title = "${repo}: Backup Started";
    })];
    before_prune  = [(mkBorgmaticNotification {
      title = "${repo}: Prune Started";
    })];
    before_compact = [(mkBorgmaticNotification {
      title = "${repo}: Compact Started";
    })];
    before_check = [(mkBorgmaticNotification {
      title = "${repo}: Check Started";
    })];
    after_backup = [(mkBorgmaticNotification {
      title = "${repo}: Backup Finished";
    })];
    after_prune = [(mkBorgmaticNotification {
      title = "${repo}: Prune Finished";
    })];
    after_compact = [(mkBorgmaticNotification {
      title = "${repo}: Compact Finished";
    })];
    after_check = [(mkBorgmaticNotification {
      title = "${repo}: Check Finished";
    })];
    on_error = [(mkBorgmaticNotification {
      title = "${repo}: ERROR";
      urgency = "CRITICAL";
    })];
  };
